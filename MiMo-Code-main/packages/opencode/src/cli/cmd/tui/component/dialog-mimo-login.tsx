...
function MimoOAuthFlow(props: { url: string; instructions: string }) {
  ...
  async function onLoginSuccess() {
    ...
    // Prefere mimo-auto (gratuito); fallback para o primeiro modelo disponível
    const mimoProvider = sync.data.provider.find((p) => p.id === "mimo")
    const xiaomiModels = xiaomi ? Object.keys(xiaomi.models) : []
    if (mimoProvider && "mimo-auto" in mimoProvider.models) {
      local.model.set({ providerID: "mimo", modelID: "mimo-auto" }, { recent: true })
    } else if (xiaomiModels.includes("mimo-auto")) {
      local.model.set({ providerID: "xiaomi", modelID: "mimo-auto" }, { recent: true })
    } else if (xiaomiModels.length > 0) {
      local.model.set({ providerID: "xiaomi", modelID: xiaomiModels[0] }, { recent: true })
    }
    dialog.clear()
  }
